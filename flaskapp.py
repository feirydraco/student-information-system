...
def create_student():
    ...
    if request.method == "POST":
        ...
        values = [student["USN"], student["Name"], student["Semester"],
                  student["dob"], student["Section"], student["Class_ID"], student["Mentor"]]
...
def update_student(USN):
    ...
    if request.method == "POST":
        ...
        values = [student["USN"], student["Name"], student["Semester"], student["dob"],
                  student["Section"], student["Class_ID"], student["Mentor"], USN]
        change_db(
            "UPDATE Student SET USN=?, Name=?, Semester=?, dob=?, Section=?, Class_ID=?, Mentor=? WHERE USN=?", values)
...
def create_teacher():
    ...
    if request.method == "POST":
        ...
        values = [teacher["Teacher"], teacher["Name"],
                  teacher["Class_ID"], teacher["Sub"]]
        change_db("INSERT INTO Student VALUES (?, ?, ?, ?, ?, ?, ?)", values)
...
def update_teacher(Teacher_ID):
    if request.method == "GET":
        teacher = query_db(
            "SELECT * FROM teacher WHERE Teacher_ID=?", [Teacher_ID], one=True)
        return render_template("/teacher/update_teacher.html", teacher=teacher)
    if request.method == "POST":
        teacher = request.form.to_dict()
        values = [teacher["Teacher_ID"], teacher["Name"],
                  teacher["Class_ID"], teacher["Subject"], Teacher_ID]
        change_db(
            "UPDATE Teacher SET Teacher_ID=?, Name=?, Class_ID=?, Sub=? WHERE Teacher_ID=?", values)
...
def create_project():
    ...
    if request.method == "POST":
        ...
        values = [project["Project_No"],
                  project["Project_Topic"], project["USN"]]
...
def update_project(Project_No, USN):
    if request.method == "GET":
        project = query_db(
            "SELECT * FROM project WHERE Project_No=? and USN=? ", [Project_No, USN], one=True)
        return render_template("/project/update_project.html", project=project)
    if request.method == "POST":
        project = request.form.to_dict()
        values = [teacher["Project_No"], project["Project_Topic"],
                  project["USN"], Project_No, USN]
        change_db(
            "UPDATE Project SET Project_No=?, Project_Topic=?, USN=? WHERE Project_No=? and USN=?", values)
        return redirect(url_for("index_project"))
...
def delete_project(Project_No, USN):
    ...
    if request.method == "POST":
        change_db("DELETE FROM Project where Project_No=? and USN=?",
                  [Project_No, USN])
        return redirect(url_for("index_project"))
...
def update_test(USN, SubCode):
    if request.method == "GET":
        tests = query_db(
            "SELECT * FROM tests WHERE USN=? and SubCode=? ", [USN, SubCode], one=True)
